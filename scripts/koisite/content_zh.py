...
DEMO = {
    ...
    "heading": "不必安裝，立即試打",
    "intro": [
        "下面並非影片，而是一個真正運作中的倉頡輸入法，就在你的瀏覽器內執行。按下字根鍵組成編碼，候選字即時出現，選一個便會輸出。使用電腦的話，可以直接用實體鍵盤輸入。",
    ],
    "tries_label": "不妨試試這幾個編碼",
    "tries": (
        {"code": "hqi", "result": "我"},
        {"code": "onf", "result": "你"},
        {"code": "hda", "result": "香"},
    ...
    "fallback": [
        "此示範需要啟用 JavaScript 才能運作。它是全站唯一一段程式碼，而且不會連接任何伺服器。",
        "編碼對照：`hqi` 是「我」，`onf` 是「你」，`hda` 是「香」，`etcu` 是「港」。",
    ],
    "note": [
        "**與 app 相同**：鍵位排列、字根、候選次序（包括粵語字加權）、空白鍵輸出第一個候選、五碼上限，以及未開始組字時候選區顯示數字行。",
        "**示範沒有的部分**：滑行與雙指和弦輸入、學習紀錄、另外四種輸入法，以及 `z` 萬用字元。字表亦收窄至 3,030 個常用字，並非 app 內完整的 27,584 個。",
    ],
}
...
LANDING = {
    ...
    "sections": [
        DEMO,
        {
            ...
            "id": "input-methods",
            "heading": "五種輸入方式，一個鍵盤",
            "intro": "不必為了轉換輸入法，而在系統鍵盤清單中反覆翻找。五種模式都在 KOI 之內，切換即時完成。",
            "columns": 3,
            "items": [
                {
                    "title": "倉頡",
                    "meta": "字形",
                    "body": "原生鍵位排列，鍵帽同時顯示英文字母與倉頡字根。候選字按字頻排序，你經常選用的字會自動往前排。",
                },
                {
                    "title": "速成",
                    "meta": "字形",
                    "body": "首尾碼輸入。與倉頡共用同一套鍵位，不需要重新記憶鍵位。",
                },
                {
                    "title": "筆劃",
                    "meta": "字形",
                    "body": "五鍵筆劃輸入，支援 `*` 萬用字元——忘記中間幾劃，輸入一個星號一樣找得到。",
                },
                {
                    "title": "粵拼",
                    "meta": "拼音",
                    "body": "全拼與簡拼皆可，並可加上聲調數字收窄結果。輸出港式繁體中文。",
                },
                {
                    "title": "注音（大千）",
                    "meta": "拼音",
                ...
                },
                {
                    "title": "英文",
                    "meta": "同步解碼",
                    "body": "同一串按鍵會同時解碼為中文與英文候選，輸入英文不需要切換鍵盤。筆劃模式下停用。",
                },
            ],
        },
        {
        ...
        },
        {
            "kind": "cards",
            "id": "feel",
            "heading": "怎樣打都可以，不必遷就鍵盤",
            "intro": "同一個組字之內，四種輸入方式可以混合使用。打到一半改變主意，也不需要清空重來。",
            "columns": 2,
            "items": [
                {
                    "title": "點按、滑行、混合、雙指和弦",
                    "body": "可以逐鍵點按，可以一筆滑過整串編碼，也可以先點按再接著滑行。兩隻手指同時按下也一併接收，全部歸入同一個組字。",
                },
                {
                    "title": "滑行略有偏移仍然接得住",
                    "body": "滑行解碼容許最多兩個鍵被相鄰鍵取代。手指稍為偏移，輸入不會因此中斷。",
                },
                {
                    "title": "打錯碼不會落空",
                    "body": "拆錯字時，KOI 會補上近似碼的候選，排在正常候選之後。候選區不會留下一片空白，讓你乾等。",
                },
                {
                    "title": "刪除與游標",
                    "body": "按一下刪去最後一個碼，連按兩下清除整個組字，長按不放則會加速。在空白鍵上左右掃動，可以移動游標。",
                },
            ],
        },
        {
            ...
            "id": "handwriting",
            "heading": "手寫，不需要連網",
            "intro": "手寫辨識模型直接封裝在 app 之內，不需要下載、不需要連線，飛航模式下一樣寫得到。",
            "columns": 2,
            "items": [
                {
                    "title": "港式手寫模型",
                    "body": "採用的是香港繁體手寫模型，而非普通話或簡體模型。你寫慣的字形，它認得出來。",
                },
                {
                    "title": "水池筆跡",
                    "body": "手寫區以 Metal 渲染成一池水：落筆有漣漪、有折射、有波紋。強度可在設定中調校，想要安靜一點就調低。",
                },
            ],
        },
        {
            ...
            "id": "agent",
            "heading": "KOI Agent",
            "intro": "需要時才開啟，不開啟就完全不存在。六種模式全部在鍵盤上完成，不必跳出去其他 app。",
            "columns": 3,
            "items": [
        ...
        CREDIT_COSTS,
        {
            ...
            "body": [
                ...
                "文字只會在一種情況下傳出：你主動開啟 KOI Agent 並送出請求。除此之外沒有例外。",
            ],
            "bullets": [
                "**沒有**分析工具、**沒有**崩潰回報 SDK、**沒有**廣告識別碼",
                "**不會**記錄你的按鍵",
                "學習紀錄與自訂設定留在裝置上，或存放於由你自己控制的私人 iCloud",
        ...
        },
        {
            ...
            "id": "plus",
            "heading": "KOI Plus",
            "intro": "免費版本已經包含五種輸入法、手寫、學習與全部基本輸入功能。Plus 另加兩項功能，並附送每月 Credits。",
            "columns": 2,
            "items": [
                {
                    "title": "聯想輸入",
                    "body": "輸出一個字之後，接著推薦下一個詞或整句短語，減少逐字拆碼。",
                },
                {
                    "title": "跨裝置同步",
                    "body": "設定與學習紀錄經你自己的私人 iCloud 同步。預設關閉；開啟之後由你手動按下同步，不會在背景悄悄傳送。",
                },
            ],
        },
        {
            ...
            "items": [
                {
                    "term": "訂閱附送",
                    "detail": "月費與年費計劃每個月發放 250 Credits。年費計劃是全年分十二次發放，而非一次全數發放。每批由發放日起 60 日內有效。",
                },
                {
                    "term": "另行購買",
                    "detail": "Credit 包分 100、300、800 三種，屬一次性購買，**不會過期**，但不包括聯想輸入與跨裝置同步。",
                },
                {
                    "term": "扣費時機",
                    "detail": "只有在你要求 KOI Agent 執行操作時才會扣減。日常打字不涉及 Credits。",
                },
            ],
        },
        {
            ...
            "items": [
                ...
                {
                    "term": "完整取用權限",
                    "detail": "AI 功能與剪貼簿貼上需要開啟「允許完整取用」。不開啟的話，五種輸入法、候選字、學習與手寫全部照常運作。詳情見[支援頁](route:support/)。",
                },
                {
                    "term": "字元覆蓋",
                    "detail": "候選字覆蓋基本多文種平面（BMP）內的漢字。部分極罕用的擴充區字（Ext B 以上）暫時未能輸入。",
                },
...
PRIVACY = {
    ...
    "sections": [
        ...
        },
        {
            ...
            "heading": "「完整取用」權限",
            "body": [
                "iOS 的規定是：鍵盤擴充功能如果沒有開啟「允許完整取用」，便完全沒有網絡能力。因此 KOI 需要這項權限來完成以下三件事：",
            ],
            "bullets": [
                "連接 KOI Agent（AI 功能）",
            ...
            "after": [
                "不開啟這項權限，KOI 的五種輸入法、候選字、學習、手寫、符號與游標操作全部照常運作。你失去的只有上述三項。",
        ...
        },
        {
            ...
            "bullets": [
                "你輸入的指令文字",
                "你當時所在輸入框的周邊文字，**上限 2,000 個 UTF-16 單位**——並非整份文件，只是游標附近的一段",
                "同一次對話之中先前的來回內容",
                "一個安裝識別碼（見下文「識別碼」一節）與一個 Firebase App Check 驗證權杖",
            ],
            "after": [
                "**不會**傳送的項目：截圖（KOI 沒有截圖功能）、你的剪貼簿內容（除非你自己按下「貼上」）、通訊錄，以及其他 app 的內容。",
        ...
        },
        {
            ...
            "body": [
                ...
                "**以 Apple 帳戶登入**：屬選用功能，只有在你希望跨裝置保留購買紀錄時才需要。KOI 向 Apple 索取的範圍**只有姓名**，並無索取電郵地址。伺服器上的帳戶記錄只存一個隨機的範圍雜湊值與時間戳，沒有姓名、沒有電郵。",
    ...
    "sections": [
        {
            ...
            "items": [
                ...
                {
                    "title": "（選用）開啟完整取用",
                    "body": "在同一頁選擇 KOI，開啟「允許完整取用」。這項權限只有 AI 功能與剪貼簿貼上才需要；不開啟的話，輸入法本身完全正常。",
                },
                {
                    "title": "切換到 KOI",
            ...
            "items": [
                {
                    ...
                    "answer": [
                        "iOS 規定第三方鍵盤如果沒有這項權限，就完全沒有網絡能力。KOI 需要它來連接 AI 功能、讀取剪貼簿（只在你按下「貼上」的一刻）以及更新手寫模型。",
                        "不開啟完全沒有問題。五種輸入法、候選字、學習、手寫、符號與游標操作全部照常使用，你只是無法使用 AI 與貼上。",
                    ],
                },
                {
                    ...
                    "answer": [
                        ...
                        "唯一例外是你主動開啟 KOI Agent 送出請求時，會傳送你的指令，以及游標附近最多 2,000 個 UTF-16 單位的文字。詳情見[私隱政策](route:privacy/)。",
                    ],
                },
                {
